        ...
        <div className="max-w-7xl mx-auto">
            ...
            >
                ...
                <div className="relative border-b pb-2 mb-4">
                    ...
                    <button
                    onClick={() => setIsOpen(false)}
                    className="absolute right-0 top-0 text-gray-400 hover:text-gray-900"
                    >
                    ✕
                    </button>
                </div>
            ...
            {siswa.map((s) => (
                <div
                key={s.id}
                className="bg-white border rounded-2xl shadow p-4 flex flex-col items-center transition hover:shadow-lg relative"
                >
                <div className="absolute right-2 top-2">
                    <button
                    onClick={() => setDropdownOpen(dropdownOpen === s.id ? false : s.id)}
                    className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-500"
                    >
                    ⋮
                    </button>
                    {dropdownOpen === s.id && (
                    <div className="absolute right-0 mt-2 w-40 bg-white border rounded-lg shadow-lg text-sm z-10">
                        <ul className="py-2">
                        <li>
                ...
                <h2 className="font-bold text-lg text-gray-800">{s.nama}</h2>
